...
const TOUR_STEPS = [
  {
    ...
    content:
      "Navigate between different project categories. You can view all projects, or focus on specific types like Web Apps or AI/ML.",
  },
  {
    selector: '[data-tour-id="sidebar-tags"]',
    title: "Filter by Tags",
    content:
      "Refine your search by selecting specific tags. You can combine multiple tags to narrow down the results.",
  },
  {
    selector: '[data-tour-id="sidebar-techs"]',
    title: "Filter by Technology",
    content:
      "Filter projects by the technologies used. This is great for seeing examples of my work with a particular framework or language.",
  },
  {
    selector: '[data-tour-id="hobbies-toggle"]',
    title: "Hobbies Section",
    content:
      "Click here to switch to a special view for my hobby projects. It even has its own color theme!",
  },
  {
    selector: '[data-tour-id="search-bar"]',
    title: "Search Projects",
    content: "Use the search bar to find projects by name or description.",
  },
  {
    selector: '[data-tour-id="view-switcher"]',
    title: "Switch Views",
    content:
      "Toggle between a visual grid view and a compact list view to browse projects.",
  },
  {
    selector: '[data-tour-id="project-grid"]',
    title: "Project Grid",
    content:
      "This is the main area where projects are displayed. Click on any project to see more details.",
  },
  {
    selector: '[data-tour-id="project-detail"]',
    title: "Project Details",
    content:
      "Detailed information about the selected project will appear here, including descriptions, image galleries, and links.",
  },
];
...
const SortableHeader: React.FC<{
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 text-sm font-semibold text-white/70 hover:text-white transition-colors ${className}`}
    >
      {icon}
      <span>{label}</span>
      {isActive &&
...
const ProjectsSection = React.forwardRef<HTMLElement, ProjectsSectionProps>(
  (
    ...
    const handleCloseOrMinimize = () => {
      // The ref points to the <section> element. We need to find its parent's next sibling, which is the contact section.
    ...
    const { allTags, allTechs } = useMemo(() => {
      if (isHobbiesMode) {
        const hobbyProjects = PROJECTS.filter((p) =>
          p.tags.includes(ProjectTag.Hobby)
        );
        const tags = [...new Set(hobbyProjects.flatMap((p) => p.tags))];
        const techs = [
          ...new Set(hobbyProjects.flatMap((p) => p.technologies)),
        ];
        return { allTags: tags, allTechs: techs };
      } else {
        // Default mode
        const professionalProjects = PROJECTS.filter(
          (p) => !p.tags.includes(ProjectTag.Hobby)
        );
        const tags = [...new Set(professionalProjects.flatMap((p) => p.tags))];
        const techs = [
          ...new Set(professionalProjects.flatMap((p) => p.technologies)),
        ];
        return { allTags: tags, allTechs: techs };
      }
    }, [isHobbiesMode]);
    ...
    useEffect(() => {
      const checkFiltersOverflow = () => {
        ...
        if (tagsListRef.current) {
          setShowMoreTagsButton(
            tagsListRef.current.scrollHeight > TWO_ROWS_HEIGHT_THRESHOLD_PX
          );
        }
        if (techsListRef.current) {
          setShowMoreTechsButton(
            techsListRef.current.scrollHeight > TWO_ROWS_HEIGHT_THRESHOLD_PX
          );
        }
      };
    ...
    const sortedAndFilteredProjects = useMemo(() => {
      const filtered = PROJECTS.filter((project) => {
        ...
        if (selectedCategory === "Hobbies") {
          ...
          categoryMatch =
            !isHobbyProject && project.tags.includes(ProjectTag.Personal);
        } else {
          const standardCategoryMatch =
            selectedCategory === "All Projects" ||
            project.category ===
              SIDEBAR_CATEGORIES[
                selectedCategory as keyof typeof SIDEBAR_CATEGORIES
              ];
    ...
    useEffect(() => {
      if (
        selectedProject &&
        !sortedAndFilteredProjects.find((p) => p.id === selectedProject.id)
      ) {
    ...
    const handleMouseMoveDetail = useCallback((e: MouseEvent) => {
      ...
      const maxDetailWidth =
        containerRect.width - sidebarFixedWidth - mainMinWidth;

      if (newWidth >= 320 && newWidth <= maxDetailWidth) {
        // Min/Max
    ...
    return (
      <section
        ref={ref}
        className={`h-screen w-full flex items-center justify-center snap-start transition-all duration-300 ${
          isMaximized ? "p-0" : "p-0 md:p-8"
        }`}
      >
        <Tour
          steps={TOUR_STEPS}
        ...
        <div
          ref={containerRef}
          className={`relative w-full h-full bg-neutral-dark/60 backdrop-blur-2xl border border-glass-stroke shadow-2xl flex flex-col overflow-hidden transition-all duration-1000 ease-out transform ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          } ${
            isMaximized
              ? "max-w-full max-h-full rounded-none"
              : "max-h-full md:max-h-[95vh] rounded-none md:rounded-3xl"
          }`}
        >
          {/* Title Bar */}
          <header className="group/header flex-shrink-0 h-11 hidden md:grid grid-cols-3 items-center px-5 bg-zinc-800/50 border-b border-glass-stroke">
            <div className="flex items-center gap-2.5">
              <button
                ...
                className="w-3.5 h-3.5 rounded-full bg-red-500 flex items-center justify-center text-black/60"
                aria-label="Close window"
              >
                <FiX
                  className="w-2 h-2 opacity-0 group-hover/header:opacity-100 transition-opacity"
                  strokeWidth={3}
                />
              </button>
              <button
                onClick={handleCloseOrMinimize}
                className="w-3.5 h-3.5 rounded-full bg-yellow-500 flex items-center justify-center text-black/60"
              ...
              <button
                ...
                className="w-3.5 h-3.5 rounded-full bg-green-500 flex items-center justify-center text-black/60"
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
              ...
              <main className="flex-1 flex flex-col bg-zinc-900/50 overflow-hidden">
                ...
                <div className="flex-shrink-0 flex items-center justify-between p-3 border-b border-glass-stroke">
                  <div className="flex items-center gap-2">
                    <button
                      className="p-1.5 rounded-lg bg-zinc-700/50 text-gray-300 md:hidden"
                    ...
                    <div className="hidden md:flex items-center gap-3">
                      <button
                        className="p-1.5 rounded-lg bg-zinc-700/50 text-gray-400"
                        disabled
                      >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div
                      ...
                      className="flex items-center bg-zinc-700/80 p-1 rounded-lg"
                    >
                      <button
                  ...
                  {sortedAndFilteredProjects.length === 0 ? (
                    ...
                    >
                      {sortedAndFilteredProjects.map((project) => (
                        ...
                        >
                          <div
                            className={`aspect-square w-full bg-secondary rounded-xl overflow-hidden transition-all duration-300 ring-offset-zinc-900/50 ring-offset-4 ${
                              selectedProject?.id === project.id
                                ? "ring-2 ring-accent"
                                : "hover:ring-2 ring-accent/50"
                            }`}
                          >
                            <img
                          ...
                          <span className="text-base text-white/90 break-words w-full">
                            {project.name}
                          </span>
                        </div>
                      ))}
                      ...
                      <ul>
                        {sortedAndFilteredProjects.map((project) => (
                          <li
                            key={project.id}
                            onClick={() => handleProjectSelect(project)}
                            className={`grid grid-cols-[minmax(0,_3fr)_minmax(0,_1fr)_minmax(0,_1fr)] gap-4 px-6 py-3 border-b border-glass-stroke/50 cursor-pointer transition-colors ${
                              selectedProject?.id === project.id
                                ? "bg-accent/30"
                                : "hover:bg-glass-overlay"
                            }`}
                          >
                ...
                <footer className="flex-shrink-0 flex items-center justify-end px-4 py-1.5 border-t border-glass-stroke bg-zinc-900/50">
                  <span className="text-sm text-white/50">
                    ...
                    {sortedAndFilteredProjects.length === 1 ? "item" : "items"}
                  </span>
                </footer>
              </main>
              ...
              <div
                data-tour-id="project-detail"
                style={{ width: `${detailWidth}px` }}
                className="flex-shrink-0 flex-col md:flex hidden border-l border-glass-stroke"
              >
        ...
        {viewMode === "grid" && (
          <ProjectPreview project={hoveredProject} position={previewPosition} />
        )}
      </section>
    );
  }
...
const SidebarContent: React.FC<any> = ({
  ...
  <>
    <nav data-tour-id="sidebar-nav" className="flex flex-col gap-3">
      <div>
        <h3 className="px-3 text-sm text-gray-400 font-semibold mb-2">
          Favorites
        </h3>
      ...
      <div data-tour-id="sidebar-tags">
        ...
        <div
          ...
          style={{
            maxHeight:
              showMoreTagsButton && !isTagsExpanded
                ? `60px`
                : `${tagsListRef.current?.scrollHeight}px`,
          }}
        >
              ...
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded-md transition-colors ${
                  selectedTags.includes(tag)
                    ...
                    : "bg-zinc-700/80 text-gray-300 hover:bg-zinc-700"
                }`}
              >
                {tag}
              </button>
      ...
      <div data-tour-id="sidebar-techs">
        ...
        <div
          ...
          style={{
            maxHeight:
              showMoreTechsButton && !isTechsExpanded
                ? `60px`
                : `${techsListRef.current?.scrollHeight}px`,
          }}
        >
...
const MobileFiltersPane: React.FC<any> = ({ onClose, ...props }) => (
  <div className="md:hidden fixed inset-0 z-50 bg-neutral-dark flex flex-col animate-fade-in">
    <header className="flex-shrink-0 h-14 flex items-center justify-between px-4 border-b border-glass-stroke">
      ...
      <button
        onClick={onClose}
        className="px-4 py-2 text-base rounded-lg font-semibold text-white bg-accent hover:bg-accent/80"
      >
        Done
      </button>
    </header>
...
const MobileDetailPane: React.FC<{
  ...
  return (
    <div
      className={`md:hidden fixed inset-0 z-[51] bg-neutral-dark flex flex-col transition-transform duration-300 ease-out ${
    ...
    >
      <header className="flex-shrink-0 h-14 flex items-center px-4 border-b border-glass-stroke">
        <button
          onClick={handleClose}
          className="flex items-center gap-2 text-base font-semibold text-white hover:text-accent"
        >
          <FiChevronLeft className="w-6 h-6" />
          Projects
        </button>
